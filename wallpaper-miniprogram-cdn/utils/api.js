...
function getLatestWallpapers() {
  return request(buildCdnUrl("/data/mobile/latest.json")).then(decodeResponse);
}

function getCategories() {
  return request(buildCdnUrl("/data/mobile/index.json")).then(decodeResponse);
}

function getWallpapers(category) {
  return request(buildCdnUrl("/data/mobile/" + encodeURIComponent(category) + ".json")).then(decodeResponse);
}

function getCategoryNames() {
  return request(buildCdnUrl("/data/mobile/index.json"), 1).then(function(data) {
    if (!data) return [];
    var categories = [];
    try {
      if (data.blob) {
  ...
  return getCategoryNames().then(function(names) {
    ...
    function loadBatch() {
      ...
      var promises = batch.map(function(name) {
        return request(buildCdnUrl("/data/mobile/" + encodeURIComponent(name) + ".json"), 1)
          .then(decodeResponse)
...
function getBingWallpapers() {
  return request(buildCdnUrl("/data/bing/latest.json")).then(decodeResponse);
}

function getDesktopWallpapers() {
  return request(buildCdnUrl("/data/desktop/latest.json")).then(decodeResponse);
}

function getDesktopCategories() {
  return request(buildCdnUrl("/data/desktop/index.json")).then(decodeResponse);
}

function getDesktopWallpapersByCategory(category) {
  return request(buildCdnUrl("/data/desktop/" + encodeURIComponent(category) + ".json")).then(decodeResponse);
}
...
function getCollectionWallpapers(items) {
  ...
  var promises = categories.map(function(cat) {
    return request(buildCdnUrl("/data/mobile/" + encodeURIComponent(cat) + ".json"), 1)
      .then(decodeResponse)
      .catch(function() { return []; });
  });
